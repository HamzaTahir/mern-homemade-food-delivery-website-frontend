...
const Order = () =>{
    ...
    const loadOrders = () =>{
        ...
        FoodlancerlistOrdersByOrderStatus(user._id, token, 'Cancelled').then(data =>{
            if(data.error){
    ...
    const showOrdersLength = () =>{
        ...
        if(orders.length === 0){
            return(
                <h4 className="text-success display-4" style={{textAlign:'center'}}>No Cancelled Orders</h4>
            )
        }
        else{
            return (
                <h4 className="text-danger display-4" style={{textAlign:'center'}}>Total Cancelled Orders: {orders.length}</h4>
            )
        }
    }
    ...
    if(loading1 === true){
        ...
        return(
            <Layout title="Cancelled Orders" description="You Can Manage Cancelled Orders Here" className="container">
                <div className="row">
                                ...
                                return(
                                    <div key={orderIndex}>
                                        {order.products.map((product, productIndex)=>{
                                            ...
                                            // setCount(count+1)
                                            // {count}
                                            if(product.foodlancer === user._id && order.status === 'Cancelled'){
                                                return(
