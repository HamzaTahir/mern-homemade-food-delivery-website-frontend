...
const Signup = () =>{
    ...
    const {name, kitchen, email, password, error, success} = values;
    ...
    const clickSubmit = (event) =>{
        ...
        signup({name, kitchen, email, password})
        .then(data =>{
            if(data.error){
    ...
    const signupForm = () =>(
        <form>
            ...
            <div className="form-group">
                <label className="text-muted">Kitchen Name</label>
    ...
    return(
        <Layout title="Foodlancer Signup" description="Node React E-Commerce App Signup" className="container col-md-8 offset-md-2">
            <div className="container col-md-8" style={{
            //   position: 'absolute', left: '50%', top: '50%',
            //   transform: 'translate(-50%, -50%)'
            }}> 
            <p style={{fontSize:27, fontWeight:600, textAlign:"center"}}>KHANSAMA Foodlancer Sign Up</p>
            {showSuccess()}
            {showError()}
            {signupForm()}
            {error} 
